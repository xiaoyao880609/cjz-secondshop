...
<body>
	...
	<div class="container">
		<div class="navbar-header">
			...
			<a class="navbar-brand" href="#" style="font-size: 25px;margin-left: -25px;">二手商城后台管理</a>
		</div>
	...
	<div class="container" style="width: 100%; padding: 0">
		<div class="col-md-2"
			style="background-color: #d2d2d2; height: 92%; padding: 0;">
			<div style="float: right; width: 100%;">
				<div id="menu1" onclick="menuBar('menu1')" class="admenu-b"
					align="center">
					<p>
						<B style="font-size: 20px; color: #5c5c5c; cursor: pointer">商品分类管理</B>
					</p>
				</div>
				...
				<div id="menu2" onclick="menuBar('menu2')" class="admenu"
					align="center">
					<p>
						<B style="font-size: 20px; color: #5c5c5c; cursor: pointer">商品管理</B>
					</p>
				</div>
							...
							<table class="table table-hover">
								<tbody id="table">
									<tr style="color: #666666">
										...
										<th style="width: 7.5%">卖家</th>
										<th style="width: 7.5%">买家</th>
									...
									<c:forEach var="order" items="${orderList}">
										<tr style="color: #666666">
											<td style="width: 10%"><a target="_blank"
																	  href="<c:url value="/admin/orderInfo?orderId=${order.id}"/>">${order.id}</a></td>
											<td style="width: 7.5%">${order.seller}</td>
											<td style="width: 7.5%">${order.customer}</td>
											<td style="width: 10%">${order.goodId}</td>
											<td style="width: 12%">${order.goodName}</td>
											<td style="width: 10%">${order.money}</td>
											<td style="width: 20%">${order.submitDate}</td>
											<c:choose>
												<c:when test="${order.statusId == 1}">
													<td style="width: 10%;color: red">交易取消</td>
												</c:when>
												<c:otherwise>
													<td style="display: ${order.statusId == 2 ? "":"none"}">交易中</td>
													<td style="display: ${order.statusId == 3 ? "":"none"};color: blue">交易完成</td>
												</c:otherwise>
											</c:choose>
											<td style="width: 10%"><a
	...
	<div class="footer1" style="background-color: #f9f9f9">
	<p>
		<a target="_blank"
    ...
    function addFirstType() {
		const name = document.getElementById("addfirst").value;
		const newFirstType = {
			"name": name
		};
		$.ajax({
            type:"POST",
            url:"/cjz/type/firstType/create",
            ...
            success:function (data) {
                ...
                if (data === false){
                    ...
                    $.each(data, function (i, type) {
						const firstId = type.id;
						const firstName = type.name;
						const firstType = "<div onclick='firstButton(" + firstId + ")' id='" + firstId + "' class='col-md-9 adUlLi r'>" + firstName + "</div>" +
    ...
    function addSecondType() {
        ...
        $.ajax({
            type:"POST",
            url:"/cjz/type/secondType/create",
            contentType: "application/json", //必须这样写
            dataType:"json",
    ...
    function deleteFirst(first_id) {
        $.getJSON("/cjz/type/firstType/delete/"+first_id, function (data) {
            if (data === false){
    ...
    function deleteSecond(second_id) {
        $.getJSON("/cjz/type/secondType/delete/"+second_id, function (data) {
            if (data === false){
                alert("删除失败，请确认该分类下是否还有商品！");
            } else if (data === "isNull") {
                document.getElementById("secondType-ba").innerHTML = "";
                alert("该分类下暂无分类！")
            } else {
    ...
    function firstButton(first_id) {
        ...
        $.getJSON("/cjz/type/secondType/"+first_id, function (data) {
            if (data === false){
                alert("好像出了问题，请重试！")
            } else if (data === "isNull") {
                alert("该分类下暂无分类！")
    ...
    function setUserStatus(statusId, userId) {
        $.getJSON("/cjz/admin/user/update/status/"+statusId+"&"+userId, function (data) {
            if (data === false){
                alert("用户状态修改失败！");
            } else {
                ...
                $.each(data, function (i, type) {
					...
					const userName = type.name;
					const userMobile = type.mobile;
					const userEmail = type.email;
					const userGender = type.gender;
					const userRegisterDate = type.registerDate;
					const userStatus = type.statusId === 4 ? "正常" : "失效";
					const opt = type.statusId === 4 ? "<a onclick='disableUser(" + userId + ")'>禁用</a>" : "<a onclick='enableUser(" + userId + ")'>激活</a>";
					const user = "<tr style=\"color: #666666\">" +
    ...
    function delUser(userId) {
        $.getJSON("/cjz/admin/user/delete/"+userId, function (data) {
            if (data === false){
                alert("用户删除失败！");
            } else {
                document.getElementById("userTable").innerHTML = "";
    ...
    function delOrder(orderId) {
        $.getJSON("/cjz/user/order/delete/"+orderId, function (data) {
            if (data){
                $.getJSON("/cjz/user/order/allOrder", function (data) {
    ...
    function delGood(goodId) {
        $.getJSON("/cjz/goods/userGoodEdit/delete/"+goodId, function (data) {
            if (data){
                $.getJSON("/cjz/admin/goods/allGoods", function (data) {
    ...
    function setStatus(goodId) {
        $.getJSON("/cjz/goods/userGoodEdit/updateGoodStatus/"+goodId, function (data) {
            if (data){
                $.getJSON("/cjz/admin/goods/allGoods", function (data) {
                    document.getElementById("goodTable").innerHTML = "";
                    $.each(data, function (i, type) {
