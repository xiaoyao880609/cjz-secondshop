...
<body>
					...
					<div class="col-md-12"
						...
						<div class="col-md-12" style="margin-top: 20px">
							<button id="success" class="btn btn-primary pull-right"
								style="display: ${orderInfo.statusId == 2 ? "":"none"}" onclick="deleteOrder()">关闭交易</button>
							<button id="noSuccess" class="btn btn-primary pull-right"
								style="display: ${orderInfo.statusId == 3 ? "":"none"}" disabled="true">交易已完成</button>
							<button id="cancelSuccess" class="btn btn-primary pull-right"
									style="display: ${orderInfo.statusId == 1 ? "":"none"}" disabled="true">交易取消</button>
						</div>
					</div>
				</div>
			</div>
				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="order" items="${orders}">
							<a href="/cjz/user/sellerInfo?orderId=${order.id}">
    ...
    function deleteOrder() {
        $.getJSON("/cjz/user/sellerOrder/delete/" + ${orderInfo.id} + "&" +${orderInfo.goodId}, function (data) {
            if (data == true){
                alert("交易关闭成功！");
